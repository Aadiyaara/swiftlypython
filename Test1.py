...
while True:
    ...
    if(max(GxA) > 2.5 or min(GxA) < -2.5):
        if flag == False:
            ...
            flag = True
            #data.append(['Index', 'Gx', 'Gy', 'Gz', 'Ax', 'Ay', 'Az'])
        else:
            i = i + 1
            data.append([i, Gx, Gy, Gz, Ax, Ay, Az])
        ...
        if flag == True:
            ...
            file_name = 'data/c_{gestureCount}.txt'.format(gestureCount = gestureCount)
            sensor_file = open(file_name, "a")
            for x in range (len(data)):
                write_string = ''
                if(x != 0):
                    write_string = '\nSTART {Ax} {Ay} {Az} {Gx} {Gy} {Gz} END'.format(Ax = data[x][3], Ay = data[x][4], Az = data[x][5], Gx = data[x][0], Gy = data[x][1], Gz = data[x][2])
                else:
                    write_string = 'START {Ax} {Ay} {Az} {Gx} {Gy} {Gz} END'.format(Ax = data[x][3], Ay = data[x][4], Az = data[x][5], Gx = data[x][0], Gy = data[x][1], Gz = data[x][2])
                sensor_file.write(write_string)
            sensor_file.close()
        #start_time = time.time()
            #file_name = '{gestureCount}.txt'.format(gestureCount = gestureCount)
            #sensor_file = open(file_name,"a")
        #print('Done')
